...
const CircularQueue<String> queue = CircularQueue<String>(CACHE_LIMIT, "mqtt-msg");
...
void processCall(String message) {
  ...
  if (id <= LIGHT_START_ADDRESS) { //all the light are started with 1000 onwards to 1999 address on jmri
    if (value == ON) {
